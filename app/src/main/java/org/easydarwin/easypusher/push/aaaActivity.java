 ...
 * 预览+推流等主页
 */
public class aaaActivity extends BaseProjectActivity implements View.OnClickListener,
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case UVC_DISCONNECT:
                    ...
                    int position = SPUtil.getScreenPushingCameraIndex(aaaActivity.this);
                    if (2 == position) {
                        position = 0;
                        SPUtil.setScreenPushingCameraIndex(aaaActivity.this, position);
                    }
                    switch (position) {
                        case 0:
...
//        }
    ...
    public void onBackPressed() {
        if (isStreaming() && SPUtil.getEnableBackgroundCamera(this)) {
            new AlertDialog.Builder(this).setTitle("是否允许后台上传？").setMessage("您设置了使能摄像头后台采集,是否继续在后台采集并上传视频？如果是，记得直播结束后," +
                ...
                aaaActivity.super.onBackPressed();
            }).setPositiveButton("退出程序", (dialogInterface, i) -> {
                for (int i1 = 0; i1 < 5; i1++) {
                    mMediaStream.stopPusherStream(i1);
                }
                aaaActivity.super.onBackPressed();
                Toast.makeText(aaaActivity.this, "程序已退出。", Toast.LENGTH_SHORT).show();
                ...
                new AlertDialog.Builder(this).setTitle("选择摄像头").setSingleChoiceItems(getCameras(),
                        getSelectedCameraIndex(), new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                if (isStreaming()) {
                                    Toast.makeText(aaaActivity.this, getPushStatusMsg() + ",无法切换摄像头",
                                            Toast.LENGTH_SHORT).show();
                                    dialog.dismiss();
                                    return;
                                }
                                SPUtil.setScreenPushingCameraIndex(aaaActivity.this, which);
                                switch (which) {
                                    case 0:
                                        mSelectCameraTv.setText("摄像头:后置");
                                        mMediaStream.switchCamera(MediaStream.CAMERA_FACING_BACK);
        ...
        if (!SPUtil.getScreenPushing(this)) {
            new AlertDialog.Builder(this).setTitle("提醒").setMessage("屏幕直播将要开始,直播过程中您可以切换到其它屏幕。不过记得直播结束后,再进来停止直播哦!").setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    SPUtil.setScreenPushing(aaaActivity.this, true);
                    onPushScreen(view);
                }
            }).show();
            return;
    ...
    private void startScreenPushIntent() {
        if (aaaActivity.mResultIntent != null && aaaActivity.mResultCode != 0) {
            Intent intent = new Intent(getApplicationContext(), RecordService.class);
            startService(intent);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                startActivityForResult(mMpMngr.createScreenCaptureIntent(), aaaActivity.REQUEST_MEDIA_PROJECTION);
            }
        }
    }
        ...
        new AlertDialog.Builder(this).setTitle("设置分辨率").setSingleChoiceItems(res_display, index,
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int position) {
                        ...
                        if (isStreaming()) {
                            Toast.makeText(aaaActivity.this, getPushStatusMsg() + ",无法切换分辨率", Toast.LENGTH_SHORT).show();
                            dialog.dismiss();
                            return;
                        }
                        ...
                        if (res_display.length > 3) {
                            //原生相机配置分辨率
                            if (!Util.getSupportResolution(aaaActivity.this).contains(title)) {
                                Toast.makeText(aaaActivity.this, "您的相机不支持此分辨率", Toast.LENGTH_SHORT).show();
                                dialog.dismiss();
                                return;
                            }
